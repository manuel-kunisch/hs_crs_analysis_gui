...
The synthetic quickstart is the **canonical end-to-end check** for a fresh HS-MOSAIC install. If anything is wrong with your setup, it will show up here. It also walks through every analysis mode (PCA, random NNMF, ROI-seeded NNMF, and fixed-H NNLS from reference spectra; see [02 Analysis modes](../tutorials/02_analysis_modes.md)) on the same data, so it doubles as a hands-on tour of the workflow.
