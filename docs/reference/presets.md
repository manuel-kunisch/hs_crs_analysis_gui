...
| `performance_settings.torch_nmf_use_compile` | boolean (default false) | v0.9.4+. Lets PyTorch pre-compile the MU update into faster code (`torch.compile()`; several small operations are merged into one pass). Most effective on NVIDIA GPUs (~1.3-2x); modest on CPU (~1.2-1.5x). Safely falls back to the normal uncompiled path when compilation is unavailable.                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `performance_settings.torch_nmf_tol` | float (default 1e-4) | v0.9.4+. Relative-improvement tolerance for the PyTorch MU NNMF solver. The solver samples the fit residual (the remaining mismatch between the data and `W × H`, see [NNMF and NNLS modes](../methods/nnmf_nnls_modes.md)) every 10 iterations and stops at the first sample where the relative drop is at or below this value (and patience is satisfied). Tighten to 1e-5 / 1e-6 for publication-grade fits; loosen to 1e-3 for fast exploration on CPU. Affects only the PyTorch MU backend — the scikit-learn NMF path keeps its own internal `tol` of 1e-4. **Legacy presets (≤ v0.9.3) that lack this key restore to the in-code default of 1e-4**, which is what earlier releases used unconditionally.          |
| `performance_settings.torch_nnls_tol` | float (default 1e-4) | v0.9.4+. Relative-step tolerance for the PyTorch FISTA NNLS solver. Each pixel chunk is checked every 10 iterations and converges when `‖a^(k+1) − a^(k)‖ / (‖a^(k)‖ + ε)` is at or below this value. Tighten to 1e-5 / 1e-6 for fixed-H NNLS publication runs where the abundance maps should be fully converged rather than an early stop; loosen to 1e-3 for fast exploration on CPU. Affects only the PyTorch NNLS backend (FISTA, an accelerated gradient solver) — SciPy's NNLS (the classic exact Lawson–Hanson algorithm) has no `tol` parameter. **Legacy presets (≤ v0.9.3) that lack this key restore to the in-code default of 1e-4**.                                                                       |
