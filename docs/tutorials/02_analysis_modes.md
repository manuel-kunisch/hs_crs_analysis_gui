...
Two symbols recur on this page and throughout the GUI: `H` holds the component spectra (one row per component) and `W` the matching concentration maps (one image per component). Together they reconstruct the data as `X ≈ WH`; the [Concepts page](../concepts.md) walks through the model.
...
The maximum reliable component count can be estimated as well: the **Diagnostics…** button next to the Components spinbox reports the effective rank of the dataset (the number of components supported above the noise floor) from an SVD with an estimated noise threshold. See [Unmixing diagnostics](../methods/unmixing_diagnostics.md).
...
|---|---|---|---|
| **NNMF solver** | The iteration scheme that improves `W` and `H` step by step: *Multiplicative Update (mu)* rescales all entries at once each iteration, *Coordinate Descent (cd)* refines one component at a time. | `mu` | `mu` is reliable and enforces non-negativity at every step. `cd` can be faster on some datasets but is less commonly needed. MU custom inits are lifted to a small `eps` internally just before the solve to avoid zero-stuck-zero; CD does not need this. See [MU init eps lift](../methods/nnmf_nnls_modes.md#non-negativity-exact-zeros-and-the-mu-init-eps-lift). |
...
| **NNLS tolerance** | 1e-4 | Relative-step tolerance for the PyTorch NNLS solver (FISTA, an accelerated gradient method), checked every 10 iterations on each pixel chunk. Convergence: `‖aₖ₊₁ − aₖ‖ / (‖aₖ‖ + ε) ≤ tol`. | **Tighten** to 1e-5 / 1e-6 for fixed-H NNLS publication runs where the abundance maps should be fully converged rather than an early stop (the exact optimum of the constrained fit, the KKT point). **Loosen** to 1e-3 for fast exploration on CPU. Affects only the PyTorch backend — SciPy's NNLS (the classic Lawson–Hanson algorithm) always solves exactly and has no `tol` parameter. Same editable-dropdown control as the NNMF tolerance. |
| **Use torch.compile (MU)** | Off | Lets PyTorch pre-compile the multiplicative-update step into faster GPU/CPU code (several small operations are merged into one pass over the data). | **Typical CUDA + Triton:** 1.3–2× on the inner loop. **CPU:** ~1.2–1.5×. **MPS / XPU:** inconsistent — `torch.compile` support on these backends is still evolving in PyTorch. If the active PyTorch build cannot compile (e.g. a CUDA build without Triton, or older MPS), the solver logs a warning and silently falls back to eager mode. First iteration pays a one-shot compile cost (~5–10 s) that equalizes across all subsequent iterations and is well worth it for 4D z/t stacks where the same shape is processed many times. |
...
!!! important "Recommended Analysis-panel settings — GPU vs CPU"
    ...
    | **NNLS tolerance** | 1e-4 | 1e-3 | 1e-5 / 1e-6 for fully converged fixed-H NNLS maps |
